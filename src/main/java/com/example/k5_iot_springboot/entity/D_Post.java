...
@AllArgsConstructor // 가 있으면 빈생성자 NoArgs 필요함
...
public class D_Post {
    ...
    @Comment("게시글 내용")
    @Column(nullable = false) // ,columnDefinition = "LONGTEXT" (MySQL에만 맞춰 쓸 거다)
    @JdbcTypeCode(SqlTypes.LONGVARCHAR) // Hibernate에게 LONGTEXT(LONGVARCHAR)로 기대할 것을 명시 (다른 DB에도 이식 가능하게 쓰고 싶다)
    @Lob // 대용량 텍스트 저장 - RDBMS에서 자동으로 TEXT(CLOB)으로 매핑 - Character Large Object
    ...
    @OneToMany(
            ...
            // : 댓글이 게시글 id를 참조한다 FK
            // : Post 내부에서는 읽기 전용 매핑, FK는 D_Comment와 연결된 테이블 가짐

            cascade = CascadeType.ALL,
            // : 뜻 : 폭포(위에서 흐른다)
            // : 부모(D_Post)에 대한 persist/merge/remove 등이 자식(D_Comment)로 전이
            // - 게시글 저장/ 삭제 시 댓글도 같이 처리
            // - 게시글 저장시 댓글이 생길수 있는 배열이 생긴다.

            orphanRemoval = true,
            // : 컬렉션에서 제거된 댓글은 고아 객체로 간주되어, DB에서도 자동 삭제 (실제 DELETE 수행)
            // 📌 고아 객체 제거와 Cascade 차이
            //cascade = CascadeType.REMOVE : 부모 삭제 시 자식도 삭제됨
            //orphanRemoval = true : 부모는 안 지워져도, 부모-자식 관계만 끊겨도 자식 삭제

            fetch = FetchType.LAZY // 게시글 목록에서 댓글 아난오게
            // : 컬렉션을 지연 로딩
            // - 댓글이 필요할 때만 실제 SELECT를 수행하여 불필요한 로딩을 방지
    )
    // 읽기 전용 // List<D_Comment> 빨간줄 나는 이유는 그 클래스에서 FK값 안해서 그럼
    private List<D_Comment> comments = new ArrayList<>(); // 게시글 댓글 모음
    // 1 : N 관계 시 컬렉션은 NPE 방지를 위해 즉시 초기화 해야함. Null Point Exception
    ...
    /*
        ...
        - 영속성 컨텍스트에 저장된 엔티티는 1차 캐시에 보관
            >> 트랜잭션이 끝날 때 실제 DB에 반영 | Service 에서 Repository을 호출
    ...
    public void removeComment(D_Comment comment) {
        if (comment == null) return;
        if (this.comments.remove(comment)) { // add 나 remove 하면 반환값 boolean 으로 나오나? 응! true/ 리스트에 없으면 false
